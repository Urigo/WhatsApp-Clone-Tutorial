...
#### [__Server__ Step 8.1: Add User type](https://github.com/Urigo/WhatsApp-Clone-Server/commit/0a76b8f5177d4644aaccac7034d5a2aa83994677)
...
```diff
@@ -47,6 +71,8 @@
 ┊47┊71┊      const message: Message = {
 ┊48┊72┊        id: messageId,
 ┊49┊73┊        createdAt: new Date(),
+┊  ┊74┊        sender: '', // TODO: Fill-in
+┊  ┊75┊        recipient: '', // TODO: Fill-in
 ┊50┊76┊        content,
 ┊51┊77┊      };
 ┊52┊78┊
```
...
#### [__Server__ Step 8.2: Resolve queries in relation to current user](https://github.com/Urigo/WhatsApp-Clone-Server/commit/d9ed7cd7c0bdcafa752f2d3ea201365a37aca999)
...
```diff
@@ -18,17 +18,35 @@
 ┊18┊18┊    recipient(message) {
 ┊19┊19┊      return users.find(u => u.id === message.recipient) || null;
 ┊20┊20┊    },
+┊  ┊21┊
+┊  ┊22┊    isMine(message, args, { currentUser }) {
+┊  ┊23┊      return message.sender === currentUser.id;
+┊  ┊24┊    },
 ┊21┊25┊  },
 ┊22┊26┊
 ┊23┊27┊  Chat: {
...
-┊25┊  ┊      // TODO: Resolve in relation to current user
-┊26┊  ┊      return null;
+┊  ┊28┊    name(chat, args, { currentUser }) {
+┊  ┊29┊      if (!currentUser) return null;
+┊  ┊30┊
+┊  ┊31┊      const participantId = chat.participants.find(p => p !== currentUser.id);
+┊  ┊32┊
+┊  ┊33┊      if (!participantId) return null;
+┊  ┊34┊
+┊  ┊35┊      const participant = users.find(u => u.id === participantId);
+┊  ┊36┊
+┊  ┊37┊      return participant ? participant.name : null;
 ┊27┊38┊    },
 ┊28┊39┊
-┊29┊  ┊    picture() {
-┊30┊  ┊      // TODO: Resolve in relation to current user
-┊31┊  ┊      return null;
+┊  ┊40┊    picture(chat, args, { currentUser }) {
+┊  ┊41┊      if (!currentUser) return null;
+┊  ┊42┊
+┊  ┊43┊      const participantId = chat.participants.find(p => p !== currentUser.id);
+┊  ┊44┊
+┊  ┊45┊      if (!participantId) return null;
+┊  ┊46┊
+┊  ┊47┊      const participant = users.find(u => u.id === participantId);
+┊  ┊48┊
+┊  ┊49┊      return participant ? participant.picture : null;
 ┊32┊50┊    },
 ┊33┊51┊
 ┊34┊52┊    messages(chat) {
```
...
-┊ 52┊   ┊    chats() {
-┊ 53┊   ┊      return chats;
+┊   ┊ 70┊    chats(root, args, { currentUser }) {
+┊   ┊ 71┊      if (!currentUser) return [];
+┊   ┊ 72┊
+┊   ┊ 73┊      return chats.filter(c => c.participants.includes(currentUser.id));
 ┊ 54┊ 74┊    },
 ┊ 55┊ 75┊
-┊ 56┊   ┊    chat(root, { chatId }) {
-┊ 57┊   ┊      return chats.find(c => c.id === chatId) || null;
+┊   ┊ 76┊    chat(root, { chatId }, { currentUser }) {
+┊   ┊ 77┊      if (!currentUser) return null;
+┊   ┊ 78┊
+┊   ┊ 79┊      const chat = chats.find(c => c.id === chatId);
+┊   ┊ 80┊
+┊   ┊ 81┊      if (!chat) return null;
+┊   ┊ 82┊
+┊   ┊ 83┊      return chat.participants.includes(currentUser.id) ? chat : null;
 ┊ 58┊ 84┊    },
 ┊ 59┊ 85┊  },
 ┊ 60┊ 86┊
 ┊ 61┊ 87┊  Mutation: {
-┊ 62┊   ┊    addMessage(root, { chatId, content }, { pubsub }) {
+┊   ┊ 88┊    addMessage(root, { chatId, content }, { currentUser, pubsub }) {
+┊   ┊ 89┊      if (!currentUser) return null;
+┊   ┊ 90┊
 ┊ 63┊ 91┊      const chatIndex = chats.findIndex(c => c.id === chatId);
 ┊ 64┊ 92┊
 ┊ 65┊ 93┊      if (chatIndex === -1) return null;
 ┊ 66┊ 94┊
 ┊ 67┊ 95┊      const chat = chats[chatIndex];
+┊   ┊ 96┊      if (!chat.participants.includes(currentUser.id)) return null;
 ┊ 68┊ 97┊
 ┊ 69┊ 98┊      const messagesIds = messages.map(currentMessage => Number(currentMessage.id));
 ┊ 70┊ 99┊      const messageId = String(Math.max(...messagesIds) + 1);
 ┊ 71┊100┊      const message: Message = {
 ┊ 72┊101┊        id: messageId,
 ┊ 73┊102┊        createdAt: new Date(),
-┊ 74┊   ┊        sender: '', // TODO: Fill-in
-┊ 75┊   ┊        recipient: '', // TODO: Fill-in
...
#### [__Server__ Step 8.3: Resolve subscriptions in relation to current user](https://github.com/Urigo/WhatsApp-Clone-Server/commit/2ba1c483e8776700402c57efc3cdb64750244763)
...
@@ -121,8 +122,16 @@
 ┊121┊122┊
 ┊122┊123┊  Subscription: {
 ┊123┊124┊    messageAdded: {
-┊124┊   ┊      subscribe: (root, args, { pubsub }) =>
-┊125┊   ┊        pubsub.asyncIterator('messageAdded'),
+┊   ┊125┊      subscribe: withFilter(
+┊   ┊126┊        (root, args, { pubsub }) => pubsub.asyncIterator('messageAdded'),
+┊   ┊127┊        ({ messageAdded }, args, { currentUser }) => {
+┊   ┊128┊          if (!currentUser) return false;
+┊   ┊129┊
+┊   ┊130┊          return [messageAdded.sender, messageAdded.recipient].includes(
+┊   ┊131┊            currentUser.id
+┊   ┊132┊          );
+┊   ┊133┊        }
+┊   ┊134┊      ),
...
#### [__Client__ Step 11.1: Distinguish messages](https://github.com/Urigo/WhatsApp-Clone-Client-React/commit/6199a5fa424b05ad99756a0eba22f94e8726b26b)
...
#### [__Client__ Step 11.2: Support credentials](https://github.com/Urigo/WhatsApp-Clone-Client-React/commit/f03ed0d0e026dcdc1e3e1ade70a77ddc98a1f0d2)
...
#### [__Server__ Step 8.4: Support credentials](https://github.com/Urigo/WhatsApp-Clone-Server/commit/5c049eb1aa413e890f74bc2ce969a7939203db65)
...
#### [__Server__ Step 8.5: Use cookie parser](https://github.com/Urigo/WhatsApp-Clone-Server/commit/ba7ba67b43ed7fe9fcad01181508b85baeef1dee)
...
#### [__Server__ Step 8.6: Define current user based on cookies](https://github.com/Urigo/WhatsApp-Clone-Server/commit/0b9bcc533c5a941237d6d4b1814ccb9582210ffc)
...
```diff
@@ -21,6 +21,7 @@
 ┊21┊21┊    "@graphql-codegen/typescript": "1.6.1",
 ┊22┊22┊    "@graphql-codegen/typescript-resolvers": "1.6.1",
 ┊23┊23┊    "@types/cors": "2.8.6",
+┊  ┊24┊    "@types/cookie": "0.3.3",
 ┊24┊25┊    "@types/cookie-parser": "1.4.2",
 ┊25┊26┊    "@types/express": "4.17.1",
 ┊26┊27┊    "@types/graphql": "14.5.0",
```
```diff
@@ -36,6 +37,7 @@
 ┊36┊37┊  "dependencies": {
 ┊37┊38┊    "apollo-server-express": "2.9.0",
 ┊38┊39┊    "apollo-server-testing": "2.9.0",
+┊  ┊40┊    "cookie": "0.4.0",
 ┊39┊41┊    "cors": "2.8.5",
 ┊40┊42┊    "cookie-parser": "1.4.4",
...
#### [__Client__ Step 11.3: Add basic auth.service](https://github.com/Urigo/WhatsApp-Clone-Client-React/commit/d9db48761d22ccf34db26b214b2730aef8ef713a)
...
#### [__Client__ Step 11.4: Add AuthScreen](https://github.com/Urigo/WhatsApp-Clone-Client-React/commit/d1120838c559e3b5e65568402c029efdac9625cb)
...
#### [__Client__ Step 11.5: Add sign-out button that ChatsNavbar](https://github.com/Urigo/WhatsApp-Clone-Client-React/commit/1faa21a5393518127e0d85c8e769f6d40c002e8a)
...
#### [__Client__ Step 11.6: Add withAuth() route wrapper](https://github.com/Urigo/WhatsApp-Clone-Client-React/commit/d1e1fd25b31f74ab7e24c5df59c8b302f54b06c2)

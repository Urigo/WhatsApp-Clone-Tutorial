...
#### [__Server__ Step 10.1: Separate app into a different module](https://github.com/Urigo/WhatsApp-Clone-Server/commit/bdcf2d118d99379d620e13e60d408482f43973a7)
...
#### [__Server__ Step 10.2: Add signIn mutation](https://github.com/Urigo/WhatsApp-Clone-Server/commit/f49f4aafef1183c1a1f4472db1a52afc512ab14c)
...
```diff
@@ -20,12 +20,14 @@
 ┊20┊20┊    "@graphql-codegen/cli": "1.6.1",
...
+┊  ┊23┊    "@types/bcrypt": "3.0.0",
 ...
 ┊26┊27┊    "@types/express": "4.17.1",
 ┊27┊28┊    "@types/graphql": "14.5.0",
 ┊28┊29┊    "@types/jest": "24.0.18",
+┊  ┊30┊    "@types/jsonwebtoken": "8.3.3",
 ┊29┊31┊    "@types/node": "12.7.2",
 ┊30┊32┊    "jest": "24.9.0",
 ┊31┊33┊    "jest-junit": "8.0.0",
```
```diff
@@ -37,6 +39,7 @@
 ┊37┊39┊  "dependencies": {
 ┊38┊40┊    "apollo-server-express": "2.9.0",
 ┊39┊41┊    "apollo-server-testing": "2.9.0",
+┊  ┊42┊    "bcrypt": "3.0.6",
 ┊40┊43┊    "cookie": "0.4.0",
 ┊41┊44┊    "cors": "2.8.5",
 ┊42┊45┊    "cookie-parser": "1.4.4",
```
```diff
@@ -44,7 +47,8 @@
 ┊44┊47┊    "graphql": "14.5.3",
 ┊45┊48┊    "graphql-import": "0.7.1",
 ┊46┊49┊    "graphql-scalars": "1.0.1",
...
@@ -92,6 +95,26 @@
 ...
 ┊ 93┊ 96┊
 ┊ 94┊ 97┊  Mutation: {
+┊   ┊ 98┊    signIn(root, { username, password }, { res }) {
+┊   ┊ 99┊      const user = users.find(u => u.username === username);
+┊   ┊100┊
+┊   ┊101┊      if (!user) {
+┊   ┊102┊        throw new Error('user not found');
+┊   ┊103┊      }
+┊   ┊104┊
+┊   ┊105┊      const passwordsMatch = bcrypt.compareSync(password, user.password);
+┊   ┊106┊
+┊   ┊107┊      if (!passwordsMatch) {
+┊   ┊108┊        throw new Error('password is incorrect');
+┊   ┊109┊      }
+┊   ┊110┊
+┊   ┊111┊      const authToken = jwt.sign(username, secret);
+┊   ┊112┊
+┊   ┊113┊      res.cookie('authToken', authToken, { maxAge: expiration });
+┊   ┊114┊
+┊   ┊115┊      return user;
+┊   ┊116┊    },
+┊   ┊117┊
 ┊ 95┊118┊    addMessage(root, { chatId, content }, { currentUser, pubsub }) {
 ┊ 96┊119┊      if (!currentUser) return null;
 ┊ 97┊120┊
```
...
#### [__Server__ Step 10.3: Get current user from auth token](https://github.com/Urigo/WhatsApp-Clone-Server/commit/55e1b222552e9f6a1d8a4c192fbda1b5b9a6096b)
...
#### [__Server__ Step 10.4: Update user schema to contain credentials](https://github.com/Urigo/WhatsApp-Clone-Server/commit/652a6c6107c6e8af57cddcd27626278a46cddd01)
...
#### [__Client__ Step 13.1: Use authToken cookie](https://github.com/Urigo/WhatsApp-Clone-Client-React/commit/711bb0a042bd874b6b8e998ade9faf3dfe8ea612)
...
#### [__Client__ Step 13.2: Update auth service to call signIn mutation](https://github.com/Urigo/WhatsApp-Clone-Client-React/commit/6ff4f12e3a06cd5081e85558b1fe7e011dffa83d)
...
#### [__Server__ Step 10.5: Add Query.me](https://github.com/Urigo/WhatsApp-Clone-Server/commit/54bf075e77be40631779d24f13b94b4d082b127b)
...
@@ -71,6 +71,10 @@
 ...
 ┊72┊72┊
 ┊73┊73┊  Query: {
+┊  ┊74┊    me(root, args, { currentUser }) {
+┊  ┊75┊      return currentUser || null;
+┊  ┊76┊    },
+┊  ┊77┊
 ┊74┊78┊    chats(root, args, { currentUser }) {
 ┊75┊79┊      if (!currentUser) return [];
 ┊76┊80┊
```
...
#### [__Client__ Step 13.3: Validate auth token against the back-end on restricted route](https://github.com/Urigo/WhatsApp-Clone-Client-React/commit/33abd7e00bf6f961081ea88f032097e244002dfe)
...
#### [__Client__ Step 13.4: Add username and password to AuthScreen](https://github.com/Urigo/WhatsApp-Clone-Client-React/commit/1da187fdc262038af3c2894bf9fd7c3072a6a8ae)
...
#### [__Server__ Step 10.6: Add signUp mutation](https://github.com/Urigo/WhatsApp-Clone-Server/commit/f53c49040fb01aae4d9a954ac10a51e8a8dad485)
...
+┊   ┊124┊      validateLength('req.name', name, 3, 50);
+┊   ┊125┊      validateLength('req.username', username, 3, 18);
+┊   ┊126┊      validatePassword('req.password', password);
+┊   ┊127┊
+┊   ┊128┊      if (password !== passwordConfirm) {
+┊   ┊129┊        throw Error("req.password and req.passwordConfirm don't match");
+┊   ┊130┊      }
+┊   ┊131┊
+┊   ┊132┊      if (users.some(u => u.username === username)) {
+┊   ┊133┊        throw Error('username already exists');
+┊   ┊134┊      }
+┊   ┊135┊
+┊   ┊136┊      const passwordHash = bcrypt.hashSync(password, bcrypt.genSaltSync(8));
+┊   ┊137┊
+┊   ┊138┊      const user: User = {
...
+┊   ┊149┊    },
+┊   ┊150┊
 ┊122┊151┊    addMessage(root, { chatId, content }, { currentUser, pubsub }) {
 ┊123┊152┊      if (!currentUser) return null;
 ┊124┊153┊
```
...
#### [__Client__ Step 13.5: Add signUp() method to auth.service](https://github.com/Urigo/WhatsApp-Clone-Client-React/commit/8d53497dda75bd48300258c30693487d526a8a7b)
...
#### [__Client__ Step 13.6: Split AuthScreen into SignInForm and SignUpForm](https://github.com/Urigo/WhatsApp-Clone-Client-React/commit/6d8d2d6e823de60ad99a34b3a90ff63ce03e4e5b)
